...
{   // ENVIAR VELOCIDAD CERO CUANDO PRESIONAMOS STOP
...
void MainWindow::on_Timer()
{
    ...
    handleGamePadStatus();
    gamePadControl(); //Leer el gampad fisico o usar el virtual
...
void MainWindow::handleGamePadStatus()
{
    ...
    if(ui->rb_arcade->isChecked()){
        gamepadmode=ARCADE;
        ui->joyPadR->setEnabled(false);
    }else {
        gamepadmode=INDIVIDUAL;
        ui->joyPadR->setEnabled(true);
    }

}
void  MainWindow::gamePadControl()
{   // si no hay ninguna gamepad conectado ve a virtual 
...
void MainWindow::virtualGamePadControl()
{
    ...
    switch(gamepadmode){
        case ARCADE:
            message.linear.x = vl_max * static_cast<double>(yL);  // Adelante
            message.angular.z= va_max*static_cast<double>(-xL);//orientacion
            break;
        case INDIVIDUAL:
            message.linear.x = vl_max*static_cast<double>((yL+yR)/2);
            message.angular.z = va_max*static_cast<double>((yR-yL)/2);
        break;
